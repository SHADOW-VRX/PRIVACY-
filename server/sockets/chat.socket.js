...
module.exports = (io, socket) => {
    ...
    socket.on('create_room_with_id', (data) => {
        ...
        // Broadcast updated user list to everyone in the room (just the creator for now)
    ...
    socket.on('join_room', (data) => {
        ...
        const usersBefore = roomService.getRoomUsers(roomId);
        ...
        socket.to(roomId).emit('user_joined', {
            ...
            users: usersBefore.map(u => ({ userId: u.userId, username: u.username }))
        });
        ...
        socket.emit('room_joined', {
            ...
            username: currentUsername,
            userCount: roomService.getUserCount(roomId),
            users: roomService.getRoomUsers(roomId).map(u => ({ userId: u.userId, username: u.username })),
        ...
        if (currentRoomId) {
            ...
            if (room) {
                ...
                socket.to(currentRoomId).emit('user_left', {
                    ...
                    users: remainingUsers.map(u => ({ userId: u.userId, username: u.username }))
                });
    ...
    // Disconnect handler (auto-cleanup)
    socket.on('disconnect', () => {
        ...
        if (currentRoomId) {
            ...
            if (room) {
                ...
                // Get remaining users before notifying
                const remainingUsers = roomService.getRoomUsers(currentRoomId);
                ...
                // Broadcast updated user list
                if (roomService.getUserCount(currentRoomId) > 0) {
                    broadcastUserList(io, currentRoomId);
                }
